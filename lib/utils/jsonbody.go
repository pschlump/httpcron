...
func JsonBody(w http.ResponseWriter, r *http.Request, cfg *config.Config, data interface{}) error {
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Invalid default data in %T", data), http.StatusBadRequest)
		// should log
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Invalid Data: %s", err), http.StatusUnprocessableEntity)
		// should log
